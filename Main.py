...
def startprint():
    ...
    print(f'''{Fore.RESET}
                      ...
                      ░██▄▄▄▄██ ▒██░    ▓▓█  ░██░▒▓▓▄ ▄██▒░██▄▄▄▄██ ▒██▀▀█▄  ░▓█▄   ▌
                       ...
                       {Fore.CYAN}Creator: {Fore.GREEN}vicious#1337
    '''+Fore.RESET)
...
@Alucard.command()
async def delete(ctx, amount: int): # b'\xfc'
    await ctx.message.delete()
...
async def help(ctx): # b'\xfc'
  await ctx.message.delete()
  embed = discord.Embed(color=(0xffffff),description=f"**Alucard Selfbot**")
  embed.add_field(name="``general``", value="**General Commands**", inline=True)
  embed.add_field(name="``fun``", value="**Fun & Social Commands**", inline=True)
  embed.add_field(name="``account``",value="**Account Commands**")
  embed.add_field(name="``hack``",value="**Hacking Commands**")
  embed.add_field(name="``raid``",value="**Raiding Commands**")
  embed.add_field(name="``util``",value="**Utility Commands**")
  embed.add_field(name="``other``",value="**Other Commands**")
  embed.add_field(name="``nsfw``", value="**NSFW Commands**", inline=False)
  embed.set_image(url='https://media.discordapp.net/attachments/799277037238878228/807399699954663424/ALUCARD-RESIZED.png')
  await ctx.send(embed=embed)  
...
async def nsfw(ctx):
  ...
  embed.description=f"``lesbian``, ``lewdjoke``, ``blowjob``, ``tits``, ``boobs``, ``hentai``, ``feet``, ``erofeet``, ``anal``"
  await ctx.send(embed=embed)
...
async def fun(ctx):
  ...
  embed.description=f"``fox``, ``dog``, ``cat``, ``minesweeper``, ``dick``, ``rainbow``, ``8ball``, ``joke``, ``slot``, ``topic``, ``wyr``, ``feed``, ``tickle``, ``slap``, ``hug``, ``smug``, ``pat``, ``kiss``"
  await ctx.send(embed=embed) 
...
async def hack(ctx):
  await ctx.message.delete()
  embed = discord.Embed(color=0xffffff)
  embed.title=("Hacking Commands")
  embed.description=f"``tokeninfo``, ``tokenfuck``, ``geoip``, ``ebay-view``, ``gmail-bomb``, ``nitro``, ``proxies``, ``address``, ``masslogin``"
  await ctx.send(embed=embed) 
...
async def raid(ctx):
  await ctx.message.delete()
  embed = discord.Embed(color=0xffffff)
  embed.title=("Raiding Commands")
  embed.description=f"``destroy``, ``dmall``, ``massban``, ``masskick``, ``massrole``, ``masschannel``, ``delroles``, ``delchannels``, ``massunban``"
  await ctx.send(embed=embed) 
...
async def util(ctx):
  ...
  embed.description=f"``bitly``, ``tinyurl``, ``weather``, ``backup-f``, ``auto-bump``, ``mac``, ``copy``, ``encode``, ``decode``, ``btc``, ``eth``"
  await ctx.send(embed=embed) 
...
async def other(ctx):
  await ctx.message.delete()
  embed = discord.Embed(color=0xffffff)
  embed=discord.Embed(title="Other Commands", url="https://github.com/vicious1337", description="Work in progress, follow **vicious1337** on Github to stay updated")
  await ctx.send(embed=embed) 
...
async def account(ctx):
  await ctx.message.delete()
  embed = discord.Embed(color=0xffffff)
  embed.title=("Account Commands")
  embed.description=f"``set-pfp``, ``btcstream``, ``pfpsteal``, ``blank``, ``hypesquad``, ``fakenet``, ``steal-all-pfp``, ``stream``, ``watching``, ``listening``, ``playing``, ``masscon``"
  await ctx.send(embed=embed)
...
async def general(ctx):
  ...
  embed.description=f"``av``, ``revav``, ``whois``, ``role-hexcode``, ``guildicon``, ``roleinfo``, ``cls``, ``logout``, ``dm``, ``everyone``, ``empty``, ``get-hwid``, ``secret``, ``bold``, ``unflip``, ``tableflip``, ``lenny``, ``shrug``, ``reverse``, ``ascii``, ``read``, ``group-leaver``,  ``delete``, ``uptime``, ``hastebin``, ``first-message``, ``abc``, ``devowel``, ``1337-speak``, ``combine``, ``pingweb``, ``spam``, ``clear``, ``tts``, ``upper``, ``lmgtfy``, ``genname``"
  await ctx.send(embed=embed)
...
@Alucard.command()
async def playing(ctx, *, message): # b'\xfc'
    await ctx.message.delete()
    game = discord.Game(
        name=message
    )
